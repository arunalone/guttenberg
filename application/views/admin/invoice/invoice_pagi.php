            ...
            <div class="tile-body">
                <?php if (!empty($list_res)) { ?>
                    <table class="table table-hover table-bordered" id="sampleTable">
                        <thead>
                            <tr>
                                <th class="sorting header_menu number_field" onclick="sortTable(this, 'inv_id')">Invoice #</th>
                                <th class="sorting header_menu number_field" onclick="sortTable(this, 'inv_total')">Invoice Amount</th>
                                <th class="sorting header_menu" onclick="sortTable(this, 'inv_created_on')">Invoice Date</th>
                                <th class="sorting header_menu" onclick="sortTable(this, 'inv_due_Date')">Invoice Due Date</th>
                                <th class="sorting header_menu" onclick="sortTable(this, 'store_name')">Store Name</th>
                                <th class="sorting header_menu" onclick="sortTable(this, 'inv_payment_status')">Status</th>
                        ...
                        <tbody>
                            ...
                            foreach ($list_res as $key => $row) {
                                ?>
                                <tr>
                                    <td class="number_field"><?php echo $row->inv_id; ?></td>
                                    <td class="number_field"><?php echo $row->inv_total; ?></td>
                                    <td ><?php echo date('m/d/Y', strtotime($row->inv_created_on)); ?></td>
                                    ...
                                    <td>
                                        <div class="toggle-flip">
                                            <label>
                                                <input onclick="updateInvoiceStatus('<?php echo $row->inv_payment_status; ?>', '<?php echo $row->inv_id; ?>')" type="checkbox" id="inv_status" value="" <?php if ($row->inv_payment_status === "PAID") { ?>checked="" disabled=""<?php } ?>><span class="flip-indecator" data-toggle-on="PAID" data-toggle-off="UNPAID" style="font-size: 13px;"></span>
                                            </label>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="javascript:invoiceDetail('<?php echo $row->inv_id; ?>')" data-toggle="tooltip" title="View" class="btn btn-info yellow"><i class="fa fa-eye mr-0" aria-hidden="true"></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
